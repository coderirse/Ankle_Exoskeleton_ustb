...
inline void can_init(void) {
    ...
    // 2026-08-05: 显式设置轮廓加减速。
    // 2026-08-06: 改与老程序一致 0x3FFFFFFF (≈关闭内部斜坡, 速度阶跃直接生效);
    // 50000 换算仅 ~83°/s², 爬到600°/s需7秒 — 实测收放线都追不上步态
    BYTE set_acc[]={0x23,0x83,0x60,0x00,0xFF,0xFF,0xFF,0x3F}; can_send(0x653,set_acc,8); usleep(50000);
    BYTE set_dec[]={0x23,0x84,0x60,0x00,0xFF,0xFF,0xFF,0x3F}; can_send(0x653,set_dec,8); usleep(50000);
    BYTE set_qdec[]={0x23,0x85,0x60,0x00,0xFF,0xFF,0xFF,0x3F}; can_send(0x653,set_qdec,8); usleep(50000);
