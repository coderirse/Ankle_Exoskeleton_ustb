...
struct PIDController 
{
    ...
    double compute(double setpoint, double measurement, double dt) {
        ...
        output = clamp(output, V_min, V_max);  // 输出限幅
...
void AdaptiveSpeed(double x)
{
  ...
  //根据实际情况调整支撑相3的参数
  Tor.DRIVE_DURATION = clamp(Tor.DRIVE_DURATION , min_duration , max_duration);
  if(ST.recorded)
  {
...
void processHoming() 
{
    ...
    if (ST.need_homing && !is_homing) 
    {
        ...
        usleep(50000);
        while(abs(Enc.Encoder_Value - Enc.initialEncoderValue) < 2 && ros::ok()) 
        {   // 关闭流程
...
//支撑相阶段1时间回调函数
void one_support_timeCallback(const std_msgs::Float64::ConstPtr& msg)
{
    timer.time1 = msg->data;
}

//支撑相阶段2时间回调函数
void two_support_timeCallback(const std_msgs::Float64::ConstPtr& msg)
{
    timer.time2 = msg->data;
}

//支撑相阶段3时间回调函数
void three_support_timeCallback(const std_msgs::Float64::ConstPtr& msg)
{
    timer.time3 = msg->data;
}

//摆动相时间回调函数
void swing_timeCallback(const std_msgs::Float64::ConstPtr& msg)
{
...
void updateCompensateTorque()
{
    ...
    ROS_INFO("补偿角度： %f", Tor.CompensatePosition);
}
...
void storeSlope() 
{
    ...
    Tor.TARGET_TORQUE_ASSIST = clamp(Tor.TARGET_TORQUE_ASSIST, minTorque, maxTorque);
    cout << "Slope: " << Tor.slope << endl;
    if (slope_file.is_open())
    {
        slope_file << Tor.slope << "\n";
        slope_file.flush(); // 立即写入
        ROS_INFO("坡度值已写入文件: %.2f", Tor.slope);
    }
}
...
void initializeSlopeFile()
{
    if (!slope_file_initialized)
    {
        ...
        {
            ROS_ERROR("无法打开坡度文件！路径: %s", file_path.c_str());
        }
    }
}
...
bool checkCommandSequence(uint8_t currentCommand) 
{
    ...
    switch (currentCommand) 
    {
        ...
        case 0x42:
            if (lastCommand != 0x41) 
            {
                ROS_ERROR("指令顺序错误：0x42不应在0x%02X之后！", lastCommand);
                return false;
            }
            break;
        case 0x43:
            if (lastCommand != 0x42) 
            {
                ROS_ERROR("指令顺序错误：0x43不应在0x%02X之后！", lastCommand);
                return false;
            }
            break;
        case 0x44:
            if (lastCommand != 0x43) 
            {
                ROS_ERROR("指令顺序错误：0x44不应在0x%02X之后！", lastCommand);
                return false;
            }
            break;
        default:
            ROS_ERROR("未知指令：0x%02X", currentCommand);
            return false;
            break;
    }
    return true;
...
void commandCallback(const std_msgs::UInt8::ConstPtr& msg) 
{
    ...
    if(!checkCommandSequence(command))
    {
        ...
        // 设置为站立模式（停止驱动）
        currentDriveMode = STAND_MODE;
        ROS_ERROR("安全限制触发：指令顺序错误！触发归零程序。");
        return; // 直接返回，不处理当前指令
    }
...
void getUserWeight() 
{
    ...
    cin >> user_weight;
    // 限制体重范围并计算基准扭矩
    user_weight = clamp(user_weight, 40.0, 90.0);
...
double updateTorquePeak() 
{
    ...
    Tor.TARGET_TORQUE_FLOAT = clamp(Tor.TARGET_TORQUE_FLOAT , -5.0 , 5.0);
    Tor.TARGET_TORQUE_PEAK = Tor.TARGET_TORQUE_BASE + Tor.TARGET_TORQUE_EXT + Tor.TARGET_TORQUE_ASSIST + Tor.TARGET_TORQUE_FLOAT;
    Tor.TARGET_TORQUE_PEAK = clamp(Tor.TARGET_TORQUE_PEAK , T_min , T_max);
    // 输出调试信息
    if(ST.record)
    {
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber supportime_one_sub = nh.subscribe("one_support_time", 10, one_support_timeCallback); // 订阅支撑相1时间
  ros::Subscriber supportime_two_sub = nh.subscribe("two_support_time", 10, two_support_timeCallback); // 订阅支撑相2时间
  ros::Subscriber supportime_three_sub = nh.subscribe("three_support_time", 10, three_support_timeCallback); // 订阅支撑相3时间
  ros::Subscriber swingtime_sub = nh.subscribe("swing_time", 10, swing_timeCallback); // 订阅摆动相时间
  ...
  ros::Subscriber imu_sub = nh.subscribe("imu", 10, imuCallback); //订阅imu传感器角速度值
  string twist_topic;
  nh.param("twist_topic_" , twist_topic , string("/system_speed"));
  ros::Subscriber twist_sub = nh.subscribe(twist_topic, 10, twistCallback); //订阅imu传感器值
  ...
  while (ros::ok())
  {
    ...
    torque_msg.TorqueValue = adjusted_torque;
    torque_msg.ForceSensortorque = SensorTorque;
    ...
    {  
      ...
      {   
          if (rec[j].ID == 0x000001D3)
          {
            ...
            //cout << "当前电机扭矩：" << Output_TorqueValue << endl;
            int32_t position_output = static_cast<int32_t>(combined_position);
            const double MOTOR_CIRCLE = 0x00040000; // 电机原始一圈的数值（262144）
            const int GEAR_RATIO = 100; // 减速比
            Output_Angle = static_cast<double>(position_output) * 360.0 / (MOTOR_CIRCLE * GEAR_RATIO);
            //cout << "当前电机转动角度：" << Output_Angle << endl;
          }
...
double calculateTargetPosition() 
{
    ...
    theta_d_dot = clamp(theta_d_dot , V_min , V_max);
